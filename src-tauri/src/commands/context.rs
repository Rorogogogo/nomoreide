...
use crate::core::agent_transcripts::{list_agent_transcripts, DEFAULT_TRANSCRIPT_LIMIT};
use crate::core::context_library::{
...
async fn snapshot(
    ...
    items.retain(|item| {
        query
            .kinds
            .as_ref()
            .is_none_or(|kinds| kinds.contains(&item.kind))
            && query.project_path.as_ref().is_none_or(|path| {
