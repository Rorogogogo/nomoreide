...
5. ~~**Cloudflare**~~ — done. `cloudflare-{manager,actions,auth,provider,context}.ts` plus one entry in `providers/registry.ts`. No route, no MCP tool, no client seam and no `mock-api.ts` change — which is the claim §4 made, now tested at the HTTP surface in `test/provider-routes.test.ts` ("a second provider, served by the same routes").
   ...
   **What the contract had to absorb, and did, without a signature change:**
   ...
   - *No `runtimeLogs`, no `oauth`, no `linkFile`.* Three optional things declined by provider #2, which is the first evidence the optionality was real: Pages tails runtime output over a websocket, Cloudflare has no client registration endpoint for `providers/oauth.ts` to mint a client against (see §11.1 — this became a *choice* rather than a hard limit in June 2026), and Wrangler writes no link file.
...
The stored token could not be handed to Wrangler — reading `~/.config/nomoreide/config.json` is blocked by the permission classifier — so the project was created through `wrangler login` instead. That was a free gate win: it made `cliAvailable` true and **verified Cloudflare's `cli` auth source live**, which no pass had ever reached. Wrangler's own consent screen also confirms §11.1's scope finding from the other direction — its first-party client requests colon-delimited `account:read`, `pages:write`, `zone:read`, the legacy form that self-managed clients are refused.

What passed, through our routes rather than Wrangler's: `listProjects` against a non-empty account (its first ever success), the pinned-project path (`status: "connected"` without a repo match), `listDeployments` with correct newest-first ordering and `isCurrentProduction` tracking `canonicalDeployment`, `getDeployment`, `buildLogs`, `runtimeLogs` (empty, as designed for a provider that serves none), `rollback` — which really did move production back — and the manifest's own refusal of `cancel` with a 404 naming the provider. A wrangler cross-check matched our deployment list field for field.

**§9's PATCH-merge env risk is retired, and it behaves.** Two variables created in sequence, one plain and one encrypted, across different environment sets: the second did not wipe the first. An update with `environments` omitted preserved them. Deleting one left the other intact. `getEnvValue` returned the plain value and answered the encrypted one with our sentence rather than an empty string. The project was left clean.

Four defects, none of which any test, typecheck or code read could have caught:
...
- **A serving project reported no domains at all.** Cloudflare's project `/domains` lists *custom* domains only, so a project reachable at `<name>.pages.dev` reads as having none — while Vercel's equivalent endpoint includes the assigned `*.vercel.app`, confirmed live on the same pass. `subdomain` was already parsed off the raw project and read by nothing. Fixed by composing it into `listDomains`, last, the way Vercel orders its own.
...
The fourth is the more uncomfortable one, because it is **the third instance on this branch of a field declared and never read** — after `capabilities` (item 1) and `scopeLabel` (item 2). Those two were caught by reasoning about the manifest; this one was invisible to that reasoning, because `subdomain` is not a manifest field at all, just a line in a raw-response interface. The pattern is wider than the manifest: *any* field a normalizer accepts and drops is a silent gap, and only something live points at it.

##### The residue, recorded rather than fixed
...
- **Generalizing a hardcoded name introduces a second failure mode, and item 2 shipped three instances of it.** Replacing `"Vercel"` with `{name}` moves the risk from "wrong word" to "no word": `t()` returns its template unchanged when a placeholder has no value, so `provider-setup.tsx` rendered the literal `{name} access token` and `Create a token on {name}` — visible only on the Cloudflare setup screen, since Vercel is connected and never shows it. `useCli` had the same defect and was invisible from either direction (Cloudflare has no CLI login to offer, Vercel has no setup screen to show). Nothing failed: not the build, not the locale-parity test, which checks that keys *match*, not that callers *supply* them. `test/i18n-interpolation.test.ts` now asserts that no `t()` call for a key with a placeholder is made without arguments, and that zh's placeholder set matches en's per key. This is the same lesson as `scopeLabel` one turn later — a declarative field holding display text needs a reader *and* a test that the reader is wired — and it is why gate item 4 is a live pass rather than a code read.
...
### 11.1 Cloudflare OAuth became possible, and that is the argument for not having frozen

**This document asserted, in §6 and in `cloudflare-auth.ts`, that Cloudflare's authorization server "offers neither" OIDC discovery nor dynamic client registration and that "its OAuth flow is reserved for pre-registered clients such as Wrangler itself." Half of that is now false.** On [2026-06-03](https://developers.cloudflare.com/changelog/post/2026-06-03-public-oauth-clients/) — ten weeks before this plan was written, and missed when it was — Cloudflare launched self-managed OAuth clients. Anyone can now register one, and Cloudflare explicitly supports **public clients using authorization code + PKCE (S256) with no client secret**, which is precisely the shape a self-hosted app that cannot ship a secret requires.

The claim that survives is narrower: Cloudflare still has **no dynamic client registration** — no RFC 7591 `/register` endpoint. That matters because it is the specific mechanism `providers/oauth.ts` is built on. `registerOAuthClient()` mints a client at runtime, which is what lets NoMoreIDE ship no client id at all, and it throws when a provider advertises no registration endpoint.

So adopting Cloudflare sign-in is **a change of auth model, not a new provider entry**:
...
It also sharpens gate item 4. The stale claim here was not a reasoning error; it was a fact about a vendor that expired, and no test could have caught it because nothing in-tree was wrong. Vendor assumptions decay on the vendor's schedule, which is the same argument for verifying against a live API rather than against a stub that encodes what we believed when we wrote it.
