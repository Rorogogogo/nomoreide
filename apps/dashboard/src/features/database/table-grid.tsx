...
function isSafeKeyValue(value: unknown): value is DatabaseRowKeyValue {
  return (
    value !== "••••" &&
    (typeof value === "string" ||
...
function toSqlInsert(row: Record<string, unknown>, sample: RowSample) {
...
function quoteIdentifier(value: string, engine: RowSample["engine"]) {
  return engine === "mysql"
    ? `\`${value.replace(/`/g, "``")}\``
    : `"${value.replace(/"/g, '""')}"`;
}
