...
**`target/` is cleaned by a git hook, because it reached 120 GiB.** Nine crates plus Tauri, debug and release, a binary per integration suite, and several agents building at once — it grows fast, and the disk is the smaller cost: a bloated `target/` makes builds slower and on macOS has wedged them outright. `.githooks/post-merge` and `.githooks/post-checkout` run `scripts/clean-target.mjs` **detached**, so git never blocks, and every path in the hook exits 0 — a hook that can break `git pull` is worse than a full disk. The policy is tiered: incremental caches are pruned every time (pure cache, often a third of the tree), and a full clean happens only past 40 GiB, because that one costs a cold rebuild. `npm run hooks:install` activates them in a fresh checkout (they live in `.githooks/`, so `core.hooksPath` has to be pointed at it once); `npm run clean:check` reports without touching anything, and `NOMOREIDE_SKIP_TARGET_CLEAN=1` turns the hook off for a session.
