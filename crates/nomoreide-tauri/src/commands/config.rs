...
use nomoreide_core::config::{
    BundleDef, Config, DatabaseDef, GitRepoDef, LogSourceDef, ServiceDef,
};
use tauri::State;

type PublicConfig = serde_json::Value;

fn public_config(config: Config) -> PublicConfig {
    config.public_value()
}

#[tauri::command]
pub async fn get_config(state: State<'_, AppState>) -> Result<PublicConfig, String> {
    state
        .config_store
        .load()
        .await
    ...
    if let Some(repository) = config.selected_git_repository.clone() {
        let config = state
            ...
            .set_github_credential(
                &repository,
                nomoreide_core::config::GithubCredentialSelection::Stored { host },
            )
            .await
            .map_err(|e| e.to_string())?;
