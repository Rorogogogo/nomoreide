...
const VERCEL_MANIFEST: ProviderManifest = {
  ...
  name: "Vercel",
  kind: "deploy",
  // Kept in step with `core/vercel-provider.ts` by hand, as the rest of this
  // manifest already is — the desktop core is the one caller that does not read
  // the registry. `test/provider-strings.test.ts` does not cover this copy.
  strings: {
    en: {
