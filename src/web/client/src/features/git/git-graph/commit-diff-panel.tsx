    ...
    <section className="flex min-h-0 min-w-0 flex-col bg-card border-b border-border">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-3 py-1.5">
        <div className="min-w-0">
          <h2 className="truncate text-[13px] font-semibold tracking-tight">
            {selectedFile ?? (selectedCommit ? selectedCommit.subject : t("git.graph.commitFallback"))}
          </h2>
          {selectedCommit ? (
      ...
      <div className="relative min-h-0 min-w-0 flex-1">
        {diffError ? (
          ...
          diffLoading ? (
            <div className="p-4 text-[12px] text-muted-foreground">{t("git.diff.loading")}</div>
          ) : diff ? (
            ...
            <div className="p-4">
              <Alert variant="muted" className="border-dashed p-6 text-center">
                {selectedCommit.parents.length > 1
                  ? t("git.graph.mergeNoDiff")
                  : t("git.graph.noTextualChanges")}
              </Alert>
            </div>
          )
        ) : (
