...
export function onboardRepoPrompt(url?: string): string {
  ...
  const lines = [
    ...
    "   It returns the clone path, a profile, and ranked proposals.",
    "2. Pick the best proposal. Only ask me if it's genuinely ambiguous (e.g. several",
    "   equally-likely apps in a monorepo) — otherwise choose sensible defaults.",
