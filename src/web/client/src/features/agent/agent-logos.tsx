...
// Google Antigravity full-color mark. Source: svgl.app/library/antigravity.svg
// (official asset; gradient blobs clipped by the arch-shaped mask).
export function AntigravityLogo({ className }: LogoProps) {
  return (
    <svg className={cn("size-3.5", className)} viewBox="0 0 16 15" fill="none" aria-hidden>
      <mask
        id="ag-mask"
      ...
      <defs>
        {/* Shared soft-blur filters standing in for the per-blob filters in the
            source asset (blur radii grouped to the nearest of 0.7 / 2.9 / 3.5). */}
        <filter id="ag-blur-sm" x="-200%" y="-200%" width="500%" height="500%">
          <feGaussianBlur stdDeviation="0.722959" />
        </filter>
        <filter id="ag-blur-md" x="-200%" y="-200%" width="500%" height="500%">
          <feGaussianBlur stdDeviation="2.9" />
        </filter>
        <filter id="ag-blur-lg" x="-200%" y="-200%" width="500%" height="500%">
          <feGaussianBlur stdDeviation="3.49513" />
        </filter>
      </defs>
    </svg>
  );
...
export function UnknownAgentLogo({ className }: LogoProps) {
  return <HelpCircle className={cn("size-3.5", className)} aria-hidden />;
}
