...
export function AgentTerminalDock({ currentPage = "services", git, onGitRefresh, onInsetChange, onNavigate }: { currentPage?: AgentDockPage; git?: DashboardData["git"]; onGitRefresh?: () => void; onInsetChange?: (placement: DockPlacement, size: number) => void; onNavigate?: (page: AgentDockPage) => void }) {
  ...
  useEffect(() => {
    onInsetChange?.(placement, fullScreen ? 0 : placement === "right" ? (open ? width : 36) : 36);
  }, [fullScreen, onInsetChange, open, placement, width]);
  useEffect(() => {
  ...
  function resizeStart(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const move = (nextEvent: PointerEvent) => {
      if (sideDocked) {
        setWidth(clampAgentDockWidth(window.innerWidth - nextEvent.clientX, window.innerWidth));
      } else {
        setHeight(clampAgentDockHeight(window.innerHeight - nextEvent.clientY, window.innerHeight));
      }
    };
    const cleanup = () => {
  ...
  return <>
    ...
    <div
      ...
      style={
        fullScreen
          ...
          : sideDocked
            ? { top: isTauri() ? 32 : 0, width: open ? width : 0 }
            : { height: open ? (height ?? "50vh") : 0 }
      }
    >
    ...
    <div
      ...
      onDoubleClick={() => sideDocked ? setWidth(480) : setHeight(null)}
    ...
    <div className="flex h-9 shrink-0 items-stretch border-b border-border bg-muted/25" data-agent-dock-toolbar>
      {!fullScreen ? <button
        ...
        className="grid w-8 shrink-0 cursor-grab place-items-center border-r border-border text-muted-foreground hover:bg-muted hover:text-foreground active:cursor-grabbing"
        onClick={() => {
    ...
    <div className="relative min-h-0 flex-1 bg-background" data-agent-split-container ref={splitContainerRef}>
      ...
      {tasks.map((task) => {
        ...
        return <div aria-labelledby={`agent-tab-${task.id}`} className={cn("absolute bottom-0", layoutSplit ? "top-9" : "top-0 inset-x-0", !shown && "invisible pointer-events-none")} id={`agent-panel-${task.id}`} key={task.id} onPointerDown={() => { paneIntentSequenceRef.current += 1; setFocusedPane(inRightPane ? "right" : "left"); }} role="tabpanel" style={paneStyle}><TerminalViewport active={shown} claimInitialInput={() => claimInitialInput(task.id)} displaySettings={settings?.confirmedGlobal.terminal} focused={focused} onStatusChange={(status: TerminalViewportStatus) => updateTaskStatus(task.id, { state: status.state === "connecting" ? "idle" : status.state, cwd: status.cwd, error: status.state === "error" ? status.detail : undefined })} ref={(handle) => { if (handle) viewportHandlesRef.current.set(task.id, handle); else viewportHandlesRef.current.delete(task.id); }} sessionId={task.id} /></div>;
      })}
      {open && composing ? <div aria-labelledby={`agent-tab-${COMPOSE_TAB_ID}`} className={cn("absolute bottom-0 bg-background", layoutSplit ? "top-9" : "top-0 inset-x-0")} id={`agent-panel-${COMPOSE_TAB_ID}`} onPointerDown={() => { paneIntentSequenceRef.current += 1; setFocusedPane("left"); }} role="tabpanel" style={layoutSplit ? { left: 0, right: `${100 - splitPercent}%` } : undefined}><AgentTerminalComposer capabilities={shellMode ? undefined : capabilities} onNavigate={onNavigate ? navigate : undefined} onShellMode={setShellMode} shellMode={shellMode} /></div> : null}
      ...
      {!sideDocked && canDropRight ? <section aria-label={t("dock.splitDropTarget")} className={cn("absolute bottom-2 right-2 z-30 grid place-items-center rounded-md border border-dashed border-primary/60 bg-primary/10 text-xs font-medium text-primary backdrop-blur-sm", split ? "top-11" : "top-2")} onDragOver={(event) => { event.preventDefault(); if (event.dataTransfer) event.dataTransfer.dropEffect = "move"; }} onDrop={(event) => { event.preventDefault(); dropDraggedRight(); }} style={{ width: split ? `calc(${100 - splitPercent}% - 0.75rem)` : "calc(50% - 0.75rem)" }}>{t("dock.splitDropTarget")}</section> : null}
