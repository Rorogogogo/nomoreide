...
export function AgentTerminalDock({ git, onGitRefresh }: { git?: DashboardData["git"]; onGitRefresh?: () => void }) {
  const { activeTaskId, closeTask, draft, focusNonce, onboarding, open, pendingTaskIds, provider, providers, selectProvider, setActiveTaskId, setOpen, stopTask, tasks, terminalError, updateTaskStatus } = useAgentDock();
  ...
  return <>{!open ? <button aria-label="Open agent terminal" className="fixed inset-x-0 bottom-0 z-50 flex h-9 w-full items-center gap-2 border-t border-border bg-card/95 px-3 text-left shadow-[0_-4px_18px_-14px_rgba(0,0,0,.45)] backdrop-blur" onClick={() => setOpen(true)} type="button"><Logo className="size-3.5 text-primary" /><span className="text-xs font-medium">{railProviderLabel}</span><span className="h-3 w-px bg-border" /><span className="min-w-0 flex-1 truncate font-mono text-[11px] text-muted-foreground">{active?.label ?? "New agent task"}</span>{active ? <span className={cn("font-mono text-[10px] text-muted-foreground", active.state === "running" && "text-emerald-600", active.state === "error" && "text-destructive")}><span className="sr-only">Active task status: </span>{stateLabel(active.state)}</span> : null}{tasks.length > 1 ? <span className="font-mono text-[10px] text-muted-foreground">{tasks.length}</span> : null}<ChevronUp className="size-3.5 text-muted-foreground" /></button> : null}<div aria-hidden={!open || undefined} className={cn("fixed inset-x-0 bottom-0 z-50 flex flex-col overflow-hidden border-t border-border bg-card shadow-[0_-12px_30px_-20px_rgba(0,0,0,.5)]", resizing || "transition-[height] duration-150", !open && "invisible pointer-events-none border-transparent shadow-none")} inert={!open || undefined} style={{ height: open ? (height ?? "50vh") : 0 }}>
    <div aria-hidden className="absolute inset-x-0 -top-1 z-20 h-2 cursor-ns-resize" data-agent-resize-grip onDoubleClick={() => setHeight(null)} onPointerDown={resizeStart} />
    <div className="flex h-9 shrink-0 items-stretch border-b border-border bg-muted/25">
      ...
      <div className="flex shrink-0 items-center border-l border-border px-1">
        ...
        <button aria-label="Collapse agent terminal" className="grid size-7 place-items-center rounded-sm text-muted-foreground hover:bg-muted hover:text-foreground" onClick={() => setOpen(false)} type="button"><ChevronDown className="size-3.5" /></button>
      </div>
    </div>
