...
import { ArrowUp, Link, Paperclip, SquareTerminal, X } from "lucide-react";
...
export function AgentTerminalComposer({ capabilities, onNavigate, onShellMode, shellMode = false }: { capabilities?: AgentCapabilities; onNavigate?: (page: AgentDockPage) => void; onShellMode?: (shell: boolean) => void; shellMode?: boolean }) {
  const t = useT();
  const { activeSource, clearSource, configured, createShellTask, createTask, creating, draft, focusNonce, insertPath, insertPrompt, onboarding, provider, providers, selectProvider, setDraft, setOnboarding } = useAgentDock();
