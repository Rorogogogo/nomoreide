...
function ProjectNotice({ project, ready }: { project: SettingsViewProps["activeProject"]; ready: boolean }) {
  if (project && !ready) return <div className="border-b border-border/50 bg-muted/30 px-4 py-2 text-xs text-muted-foreground">Loading settings for {project.name}…</div>;
...
export function SettingsView({ activeProject = null, onNavigate }: SettingsViewProps = {}) {
  ...
  if (settings.loading) {
    ...
    content = <div><h2 className="mb-4 text-base font-semibold">Search results</h2>{searchMatches.length ? searchMatches.map((id) => <div className="mb-5" key={id}><div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{categoryById(id).label}</div><CategoryContent activeProject={activeProject} category={id} onNavigate={onNavigate} projectReady={projectReady} settings={settings} /></div>) : <UnavailableSetting>No settings match “{search.trim()}”.</UnavailableSetting>}</div>;
  } else {
    content = <CategoryContent activeProject={activeProject} category={selected} onNavigate={onNavigate} projectReady={projectReady} settings={settings} />;
  }
...
function CategoryContent({ category, activeProject, projectReady, onNavigate, settings }: { category: SettingsCategoryId; activeProject: SettingsViewProps["activeProject"]; projectReady: boolean; onNavigate: SettingsViewProps["onNavigate"]; settings: SettingsValue }) {
  ...
  switch (category) {
    ...
    case "services-logs": return <ScopeSection scope="project"><ProjectNotice project={activeProject} ready={projectReady} /><SettingToggle checked={settings.project.logs.showTimestamps} description="Show the time gutter beside each log entry." disabled={projectDisabled} id="setting-log-timestamps" label="Show timestamps" onChange={(value) => void settings.updateProject({ logs: { showTimestamps: value } })} /><SettingToggle checked={settings.project.logs.wrapLines} description="Wrap long output instead of scrolling horizontally." disabled={projectDisabled} id="setting-wrap-lines" label="Wrap log lines" onChange={(value) => void settings.updateProject({ logs: { wrapLines: value } })} /></ScopeSection>;
    case "git-github": return <><ScopeSection scope="global" title="Global"><ManagementRow action={<Button onClick={() => onNavigate?.("github")} size="sm" type="button" variant="outline">Manage GitHub</Button>} description="Review the current GitHub connection without exposing credentials here." title="GitHub connection" /></ScopeSection><ScopeSection scope="project" title="Current project"><ProjectNotice project={activeProject} ready={projectReady} /><ManagementRow description="Repository-specific Git preferences will appear when their behavior is available." title="Repository defaults" /></ScopeSection></>;
    case "agents-mcp": return <><ScopeSection scope="global" title="Global"><ManagementRow action={<Button onClick={() => onNavigate?.("agent-env")} size="sm" type="button" variant="outline">Open Agent Environments</Button>} description="Manage installed agents, MCP servers, skills, and profiles." title="Agent environments" /></ScopeSection><ScopeSection scope="project" title="Current project"><ProjectNotice project={activeProject} ready={projectReady} /><ManagementRow description="Project-specific agent configuration remains in Agent Environments." title="Project agent context" /></ScopeSection></>;
    case "database-safety": return <ScopeSection scope="project"><ProjectNotice project={activeProject} ready={projectReady} /><SettingToggle checked={settings.project.database.confirmWrites} description="Show a danger confirmation before write statements are submitted." disabled={projectDisabled} id="setting-confirm-writes" label="Confirm before writes" onChange={(value) => void settings.updateProject({ database: { confirmWrites: value } })} /><SettingNumberInput description="Maximum rows requested for a default browse or query." disabled={projectDisabled} id="setting-result-limit" label="Default result limit" max={5000} min={10} onSave={(value) => settings.updateProject({ database: { resultLimit: value } })} value={settings.project.database.resultLimit} /><ManagementRow action={<Button disabled={projectDisabled} onClick={() => onNavigate?.("database")} size="sm" type="button" variant="outline">Open Database</Button>} description="Manage connections and write access in the Database workbench." title="Connections" /></ScopeSection>;
    ...
    case "data-privacy": return <><ScopeSection scope="global" title="Global"><ManagementRow description="UI preferences stay in local browser storage; operational settings live in your NoMoreIDE config directory." title="Local settings storage" /></ScopeSection><ScopeSection scope="project" title="Current project"><ProjectNotice project={activeProject} ready={projectReady} /><UnavailableSetting>Export and reset controls are coming in a later delivery.</UnavailableSetting></ScopeSection></>;
